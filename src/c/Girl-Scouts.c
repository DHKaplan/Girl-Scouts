...
void handle_init(void) {
  ...
  GColor BGCOLOR   = COLOR_FALLBACK(GColorJaegerGreen, GColorBlack);
  BGColorHold = BGCOLOR;

  window = window_create();
  window_stack_push(window, true /* Animated */);
  window_set_background_color(window, BGColorHold);  
  ...
  text_layer_set_background_color(text_year_layer, BGColorHold);
  ...
  text_layer_set_background_color(text_mmdd_layer, BGColorHold);
  ...
  text_layer_set_background_color(text_dayname_layer, BGColorHold);
...
// The bitmap layer holds the image for logo display
  #ifdef PBL_COLOR
      image = gbitmap_create_with_resource(RESOURCE_ID_IMAGE_GIRL_SCOUT_LOGO);
  #else
      image = gbitmap_create_with_resource(RESOURCE_ID_IMAGE_GIRL_SCOUT_LOGO_BW);  
  #endif
  #ifdef PBL_PLATFORM_CHALK
      image_layer = bitmap_layer_create(GRect(25, 53, 55, 52));
  #else
      image_layer = bitmap_layer_create(GRect(7, 50, 55, 52));
  #endif

  bitmap_layer_set_bitmap(image_layer, image);
  bitmap_layer_set_background_color(image_layer, BGColorHold);
  ...
  text_layer_set_background_color(text_time_layer, BGColorHold);
  ...
  // Green Line
  #ifdef PBL_PLATFORM_CHALK
      GRect line_frame = GRect(38, 37, 104, 2);
  #else
